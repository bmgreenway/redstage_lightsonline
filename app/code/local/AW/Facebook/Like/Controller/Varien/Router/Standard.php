...
class AW_Facebook_Like_Controller_Varien_Router_Standard extends Mage_Core_Controller_Varien_Router_Standard
{
    public function match(Zend_Controller_Request_Http $request)
    {
        ...
        if (!in_array(count($path), array(2, 3)) || ($path[0] != 'facebookilike')) {
            return parent::match($request);
        }

        // Define initial values for controller initialization
        $module              = $path[0];
        $realModule          = 'AW_Facebook_Like';
        $controller          = $path[1];
