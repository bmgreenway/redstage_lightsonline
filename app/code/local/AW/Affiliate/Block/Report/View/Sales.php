...
class AW_Affiliate_Block_Report_View_Sales extends AW_Affiliate_Block_Report_View_Abstract
{
    protected function _beforeToHtml()
    {
        ...
        $diagram = $this->getLayout()->createBlock('awaffiliate/report_view_sales_diagram');
        $diagram->setTitle($this->__('Referral Sales Report'));
        $diagram->setVAxisFormat($this->getDefaultCurrencySymbol());
        $diagram->addChartColumn('date', 'string', $this->getDetalizationLabel());
        $diagram->addChartColumn('profit', 'number', $this->__('Commission'));
        $diagram->setItems($this->getItems());
        $this->setChild('diagram', $diagram);
    ...
    protected function _saveItemsInSession()
    {
        ...
        foreach ($this->_items as $item) {
            $items[] = array(
                ...
                $this->__('CR') => $item['cr'],
                $this->__('Commission') => $item['profit']
            );
        }
