    ...
    implements Mage_Adminhtml_Block_Widget_Tab_Interface
{
    ...
    protected function _prepareForm()
    {
        ...
        if ($affiliate->hasId()) {
            $balanceFieldset = $form->addFieldset('balance_fieldset',
                array('legend' => Mage::helper('awaffiliate')->__('Commissions balance'))
            );
            ...
            $balanceFieldset->addField('current_balance', 'note', array(
                ...
                'label' => Mage::helper('awaffiliate')->__('Commissions balance'),
                'title' => Mage::helper('awaffiliate')->__('Commissions balance'),
            ));
        ...
        if ($affiliate->hasId()) {
            $addingProfitFieldset = $form->addFieldset('add_profit_fieldset',
                array('legend' => Mage::helper('awaffiliate')->__('Manual transaction'))
            );
    ...
    protected function _getScriptInitialization()
    {
        ...
        $html .= " awAffiliateProfitConfig.requestUrl = '" . $this->getUrl('awaffiliate/adminhtml_affiliate/jsonProfitAdd', array('affiliate_id' => $affiliate->getId())) . "' ; ";
        $html .= "</script>";
        return $html;
    }
