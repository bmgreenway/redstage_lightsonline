...
class AW_Affiliate_Block_Adminhtml_Affiliate_Edit_Tab_Balance_Profits_Form extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        $form->addField('profit_amount', 'text', array(
            ...
            'after_element_html' =>
                '<p class="aw-note">' . Mage::helper('awaffiliate')->__("Enter amount in base website currency. Negative values allowed and will be subtracted from the affiliate's balance.") . '</p>',
        ));
...
*/
        $form->addField('profit_notice', 'textarea', array(
            ...
            'after_element_html' => '<p class="aw-note">' . Mage::helper('awaffiliate')->__('Visible only for admin') . '</p>',
            'style' => 'height:5em',
        ));
