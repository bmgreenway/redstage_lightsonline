...
class AW_Affiliate_Model_Campaign extends Mage_Core_Model_Abstract
{
    ...
    public function getConditionsModel($reload = false)
    {
        if (is_null($this->_conditionsModel) || $reload) {
            ...
            $arr = $this->getProductSelectionRule();
            if (is_array($arr)) {
                if (isset($arr['rule']) && is_object($arr['rule'])) {
                    # old format
                ...
                } elseif (isset($arr['conditions'])) {
                    # new format
                    $arr['actions'] = $arr['conditions'];
                    $this->_conditionsModel->loadPost($arr);
                }
            }
        }
