...
class AW_Onestepcheckout_AjaxController extends Mage_Checkout_Controller_Action
{
    ...
    public function applyStorecreditAction()
    {
        ...
        if (!$this->getOnepage()->getQuote()->getItemsCount()) {
            ...
            try {
                ...
                $quote->setUseStorecredit((bool)$this->getRequest()->getParam('use_storecredit'));
    ...
    /**
     * @return AW_Giftcard_Model_Giftcard|null
     */
    ...
    public function applyAWGiftcardAction()
    {
        ...
        try {
            if (!$giftCardModel = $this->_getAWGiftCardFromRequest()) {
    ...
    public function checkAWGiftcardAction()
    {
        ...
        try {
            ...
            $gcCode = $giftCardModel->getCode();
            $gcStatus = Mage::getModel('aw_giftcard/source_product_attribute_option_yesno')
                ->getOptionText($giftCardModel->getStatus());
            $gcState = Mage::getModel('aw_giftcard/source_giftcard_status')
                ->getOptionByValue($giftCardModel->getState());
            $gcBalance = Mage::helper('core')->currency($giftCardModel->getBalance());
            $gcExpires = $giftCardModel->getExpireAt();

            $result['messages'][] = $this->__('Code: %s', $coreHelper->escapeHtml($gcCode));
            $result['messages'][] = $this->__('Active: %s', $coreHelper->escapeHtml($gcStatus));
            $result['messages'][] = $this->__('Status: %s', $coreHelper->escapeHtml($gcState));
            $result['messages'][] = $this->__('Balance: %s', $gcBalance);
            if ($gcExpires) {
                $result['messages'][] = $this->__(
                    'Expires: "%s"',
                    $coreHelper->escapeHtml($coreHelper->formatDate($gcExpires, 'short'))
                );
            }
        } catch (Mage_Core_Exception $e) {
            $result['messages'][] = $e->getMessage();
    ...
    public function removeAWGiftcardAction()
    {
        ...
        $result = $this->_getEmptyResponseArrayForGiftCards();
        try {
            if (!$giftCardModel = $this->_getAWGiftCardFromRequest(true)) {
                throw new Exception($this->__('Gift Card is not valid.'));
            }
