...
include_once Mage::getConfig()->getOptions()->getLibDir().'/twitter/twitteroauth.php';

class AW_Twitter_Connect_Controller_Varien_Router_Standard extends Mage_Core_Controller_Varien_Router_Standard
{
	public function match(Zend_Controller_Request_Http $request)
	{
		$path = explode('/', trim($request->getPathInfo(), '/'));
		// If path doesn't match your module requirements
		if(!in_array(count($path), array(2,3)) || ($path[0] != 'twitterconnect')) {
			return parent::match($request);
		}
		// Define initial values for controller initialization
		$module = $path[0];
		$realModule = 'AW_Twitter_Connect';
		$controller = $path[1];
