...
class AW_Twitter_Connect_Model_Session extends Mage_Core_Model_Session_Abstract
{
	...
	{
		$namespace = 'aheadworks_twitter_connect_' . (Mage::app()->getStore()->getWebsite()->getCode());
		$this->init($namespace);
	}
...
#        Mage::log(__METHOD__.': CHECK COOKIE START');
        ...
        if(!($synch instanceof AW_Twitter_Connect_Model_Synch) || !($oauthToken=$synch->getOauthToken())
...
#        Mage::log(__METHOD__.': CHECK COOKIE END');
 	...
 	public function twitterPost($order_details,$customer_id)
 	{
        $item = Mage::getResourceModel('twitterconnect/synch_collection')
                        ->addFieldToFilter('customer_id',$customer_id)
                        ->getFirstItem();
        if(!$item->getId()){
            return false;
        }
        $twitterId = $item->getTwitterId();
        $connection = new TwitterOAuth(
            $this->getConsumerKey(),
            $this->getConsumerSecret(),
            $item->getOauthToken(),
            $item->getOauthTokenSecret()
        );
