...
class Unirgy_DropshipBatch_Adminhtml_DistController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function massStatusAction()
    {
        ...
        try {
            ...
            $this->_getSession()->addException($e, Mage::helper('udropship')->__('There was an error while updating history(ies) status'));
        }
    ...
    public function massRetryAction()
    {
        ...
        try {
            ...
            $this->_getSession()->addSuccess(
                Mage::helper('udropship')->__('Total of %d record(s) were retried', count($modelIds))
            );
        }
        catch (Mage_Core_Model_Exception $e) {
            $this->_getSession()->addError($e->getMessage());
            ...
            $this->_getSession()->addException($e, Mage::helper('udropship')->__('There was an error while retrying distribution: '.$e->getMessage()));
        }
