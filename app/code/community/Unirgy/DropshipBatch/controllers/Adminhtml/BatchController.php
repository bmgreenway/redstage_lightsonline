...
class Unirgy_DropshipBatch_Adminhtml_BatchController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function editAction()
    {
        ...
        $this->_addBreadcrumb(Mage::helper('udropship')->__('Vendor PO Import/Export Batches'), Mage::helper('udropship')->__('Vendor PO Import/Export Batches'));
    ...
    public function saveAction()
    {
        if ( $this->getRequest()->getPost() ) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('udropship')->__('Batch was successfully saved'));
    ...
    public function deleteAction()
    {
        if (($id = $this->getRequest()->getParam('id')) > 0 ) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('udropship')->__('Batch was successfully deleted'));
                $this->_redirect('*/*/');
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($batchIds)) {
            $this->_getSession()->addError(Mage::helper('udropship')->__('Please select batch(es)'));
        }
        else {
            try {
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('udropship')->__('Total of %d record(s) were successfully deleted', count($batchIds))
                );
            } catch (Exception $e) {
    ...
    public function massStatusAction()
    {
        ...
        try {
            ...
            $this->_getSession()->addException($e, Mage::helper('udropship')->__('There was an error while updating batch(es) status'));
        }
