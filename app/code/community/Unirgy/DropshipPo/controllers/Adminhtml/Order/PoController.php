...
class Unirgy_DropshipPo_Adminhtml_Order_PoController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _initPo($forSave=true)
    {
        ...
        if (!$po->getId()) {
            $this->_getSession()->addError(Mage::helper('udropship')->__('This purchase order no longer exists.'));
            $this->_redirect('*/*/');
    ...
    {
        $this->_title(Mage::helper('udropship')->__('Sales'))->_title(Mage::helper('udropship')->__('Purchase Orders'))->_title(Mage::helper('udropship')->__('Shipments'));

        $shipment = false;

        if (!$udpo->getId()) {
            $this->_getSession()->addError(Mage::helper('udropship')->__('The po no longer exists.'));
            return false;
        }
        if (!$udpo->canCreateShipment()) {
            $this->_getSession()->addError(Mage::helper('udropship')->__('Cannot do shipment for the po.'));
            return false;
        }
	...
	public function newShipmentAction()
    {
        ...
        if ($shipment = $this->_initShipment($udpo, false)) {
            $this->_title(Mage::helper('udropship')->__('New Shipment'));
	...
	public function saveShipmentAction()
    {
        ...
        try {
            ...
            if ($shipment = $this->_initShipment($udpo, true)) {
                ...
                $this->_getSession()->addSuccess(Mage::helper('udropship')->__('The shipment has been created.'));
            	...
            	if (!empty($data['do_invoice'])) {
		        	...
		        	if (Mage::helper('udpo')->invoiceShipment($shipment)) {
		        		$session->addSuccess(Mage::helper('udropship')->__('Shipment was succesfully invoiced'));
		        	} else {
		        		$session->addError(Mage::helper('udropship')->__('Shipment was not invoiced'));
		        	}
		        }
            	...
            	$this->_getSession()->addError(Mage::helper('udropship')->__('Cannot initialize shipment.'));
            }
    ...
    protected function __initOrder()
    {
        ...
        if (!$order->getId()) {
            $this->_getSession()->addError(Mage::helper('udropship')->__('This order no longer exists.'));
            $this->_redirect('*/*/');
            $this->setFlag('', self::FLAG_NO_DISPATCH, true);
            return false;
        }
    ...
    protected function _initOrder()
    {
        ...
        if (!$poHlp->checkCreatePoQtys($order, $qtys)) {
            Mage::throwException(Mage::helper('udropship')->__('Cannot create PO with this qtys'));
        }
        $isVirtual = array();
    ...
    public function updateQtyAction()
    {
        try {
            ...
            $response = array(
                'error'     => true,
                'message'   => Mage::helper('udropship')->__('Cannot update item quantity.')
            );
            $response = Zend_Json::encode($response);
        }
    ...
    public function deleteAction()
    {
        ...
        if ($udpo = $this->_initPo(false)) {
            try {
                ...
                $this->_getSession()->addSuccess(Mage::helper('udropship')->__('PO was successfully deleted'));

            } catch (Mage_Core_Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            } catch (Exception $e) {
                Mage::logException($e);
                $this->_getSession()->addError(Mage::helper('udropship')->__('Cannot delete PO.'));
            }
            $this->_redirect('*/po');
        } else {
    ...
    public function newAction()
    {
        if ($order = $this->_initOrder()) {
            $this->_title(Mage::helper('udropship')->__('New Purchase Order'));
    ...
    public function viewAction()
    {
        if ($po = $this->_initPo(false)) {
            $this->_title(Mage::helper('udropship')->__('View Purchase Order'));
    ...
    public function saveAction()
    {
        ...
        try {
            if ($order = $this->_initOrder()) {
                ...
                $this->_getSession()->addSuccess(Mage::helper('udropship')->__('Created %s Purchase Orders', $posCreated));
                if (!empty($data['comment_text'])) {
            ...
            Mage::logException($e);
            $this->_getSession()->addError(Mage::helper('udropship')->__('Cannot save po.'));
        }
        $this->_redirect('*/*/new', array('order_id' => $this->getRequest()->getParam('order_id')));
    }
        ...
        try {
            ...
            if ($poStatus == $poStatusCanceled) {
            	if ($udpo->getCurrentlyCanceledQty()>0) {
            		$this->_getSession()->addSuccess(Mage::helper('udropship')->__('%s items were canceled', $udpo->getCurrentlyCanceledQty()));
            	} else {
            		$this->_getSession()->addNotice(Mage::helper('udropship')->__('There were no items to cancel that match requested condition'));
            	}
            	if (!$poStatusSaveRes) {
            		$this->_getSession()->addNotice(Mage::helper('udropship')->__('Status cannot be changed to canceled because po still have processing items'));
            	} else {
            		$this->_getSession()->addNotice(Mage::helper('udropship')->__('Po Status changed to canceled'));
            	}
                $response = array(
                    'ajaxExpired'  => true,
                ...
                $response = array(
                    'error'     => true,
                    'message'   => Mage::helper('udropship')->__('Cannot change status from %s to %s', Mage::helper('udpo')->getPoStatusName($udpo->getUdropshipStatus()), Mage::helper('udpo')->getPoStatusName($poStatus))
                );
                $response = Zend_Json::encode($response);
            }
        } catch (Mage_Core_Exception $e) {
    ...
    public function saveCostsAction()
    {
        ...
        if ($po = $this->_initPo(false)) {
            try {
                ...
                $this->_getSession()->addSuccess(Mage::helper('udropship')->__('Costs were successfully updated'));
