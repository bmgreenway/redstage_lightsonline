...
var AWOnestepcheckoutCoreUpdater = {
    ...
    onRequestCompleteFn: function(transport) {
        ...
        if (typeof response.blocks['centinel_frame'] != 'undefined') {
            if (response.blocks['centinel_frame'].search('centinel_authenticate_block') != -1) {
                this.popup.showPopupWithDescription(response.blocks['centinel_frame']);
                this.popup.buttons.close.onClickFn = function (e) {
                    awOSCForm.disablePlaceOrderButton();
                    location.reload();
                }
                var iframe = this.popup.contentContainer.select('#centinel_authenticate_iframe').first();
                iframe.observe('load', function () {
                    $('centinel_authenticate_block').show();
                });
            }
        }
