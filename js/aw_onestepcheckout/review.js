...
AWOnestepcheckoutReviewStorecredit.prototype = {
    ...
    applyStorecredit: function() {
        ...
        var requestOptions = {
            method: 'post',
            parameters: {
                use_storecredit: this.useStorecreditCheckbox.getValue()
            },
            onComplete: function(transport) {
                if (currentAjaxRequestId !== me.ajaxRequestId) {
                    return;
...
AWOnestepcheckoutAWGiftcard.prototype = {
    ...
    initRemoveHandle: function () {
        var me = this;
        $$(this.cancelGiftcardElSelector).each(function (el) {
            if (el.getAttribute('href') && (el.getAttribute('href').indexOf('awgiftcard/card/remove/giftcard_code') !== -1)) {
                el.observe('click', function (e) {
                    me.removeGiftcard(e, el);
                });
            }
    ...
    applyGiftcard: function (e) {
        ...
        var me = this,
            ...
            requestOptions = {
                method: 'post',
                parameters: {
                    aw_giftcard_code: this.giftcardCodeInput.getValue()
                },
                onComplete: function (transport) {
                    if (currentAjaxRequestId !== me.ajaxRequestId) {
                        return;
    ...
    removeGiftcard: function (e, el) {
        ...
        this.removeMsg();
        var me = this,
            code = el.getAttribute('href').match(/awgiftcard\/card\/remove\/giftcard_code\/([^\/]+)\//)[1],
            currentAjaxRequestId = (++this.ajaxRequestId),
            requestOptions = {
                method: 'post',
                parameters: {
